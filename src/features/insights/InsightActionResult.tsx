/**
 * InsightActionResult — Confirmation screen after applying a recommended fix.
 *
 * States:
 *  - Pre-apply: shows projected impact, action steps, and "Apply" button
 *  - Applying: shows progress bar animation
 *  - Post-apply: shows success confirmation with before/after metrics
 *
 * Data flow: Static data from ./data/actionData.ts.
 */
...
import {
  ArrowLeft, CheckCircle2, Smartphone, Zap, Clock,
  TrendingUp, ArrowRight, Sparkles, RotateCcw,
} from "lucide-react";
import { Progress } from "@/components/ui/progress";
import { actionSteps, expectedResults } from "./data/actionData";

const resultIcons = { smartphone: Smartphone, clock: Clock, "trending-up": TrendingUp };

export default function InsightActionResult() {
  ...
  const handleApply = () => {
    setApplying(true);
    setTimeout(() => { setApplying(false); setApplied(true); }, 2000);
  };

  return (
    <DashboardLayout title="Take Action">
      <div className="flex items-center gap-2">
        <button onClick={() => navigate("/insight/performance-drop")} className="flex items-center gap-1.5 text-[13px] text-muted-foreground hover:text-foreground transition-colors">
      ...
      {!applied ? (
        <>
          ...
          <div className="asana-card p-6 border-l-4 border-l-primary">
            <div className="flex items-start gap-4">
              ...
              <div>
                ...
                <h1 className="text-[22px] font-bold text-foreground mt-1">Optimize mobile landing page</h1>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {expectedResults.map((item) => {
              const Icon = resultIcons[item.iconKey as keyof typeof resultIcons];
              return (
          ...
          <div className="asana-card p-6">
            ...
            <div className="mt-6 flex items-center gap-3">
              <Button className="rounded-lg text-[14px] h-10 gap-2 px-6" onClick={handleApply} disabled={applying}>
                {applying ? (
                  <><div className="h-4 w-4 border-2 border-primary-foreground/30 border-t-primary-foreground rounded-full animate-spin" /> Applying changes...</>
                ) : (
                  <><Sparkles className="h-4 w-4" /> Apply All Changes</>
                )}
              </Button>
              <Button variant="outline" className="rounded-lg text-[14px] h-10" onClick={() => navigate("/insight/performance-drop")}>Cancel</Button>
            </div>
        ...
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {expectedResults.map((item) => {
              ...
              return (
                <div key={item.label} className="asana-card p-5">
                  <div className="flex items-center gap-2 mb-3">
                    <div className="h-8 w-8 rounded-lg bg-chart-success/10 flex items-center justify-center"><Icon className="h-4 w-4 text-chart-success" /></div>
                    <p className="text-[13px] font-medium text-muted-foreground">{item.label}</p>
                  </div>
                  <div className="flex items-baseline gap-2">
                    <span className="text-[14px] text-muted-foreground line-through">{item.current}</span>
          ...
          <div className="asana-card p-6">
            ...
            <div className="mt-6 flex items-center gap-3">
              <Button className="rounded-lg text-[14px] h-10 gap-2" onClick={() => navigate("/")}>Back to Dashboard <ArrowRight className="h-4 w-4" /></Button>
              <Button variant="outline" className="rounded-lg text-[14px] h-10 gap-2" onClick={() => setApplied(false)}><RotateCcw className="h-4 w-4" /> Undo Changes</Button>
            </div>
          </div>
        </>
      )}
