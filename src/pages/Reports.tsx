    ...
    <DashboardLayout title="Reports">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="apple-card p-5 flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-[24px] font-semibold text-foreground">{reports.length}</p>
          </div>
        </div>
        <div className="apple-card p-5 flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-[24px] font-semibold text-foreground">{scheduled.length}</p>
          </div>
        </div>
        <div className="apple-card p-5 flex items-center gap-4">
      ...
      <div className="apple-card overflow-hidden">
        <div className="p-5 border-b border-border/60 flex items-center justify-between">
          ...
          <Button variant="outline" size="sm" className="rounded-xl text-[13px] h-8">
            Generate New
          </Button>
        </div>
        <table className="w-full text-[13px]">
          ...
          <tbody>
            {reports.map((report) => (
              <tr key={report.name} className="border-b border-border/20 last:border-0 hover:bg-muted/30 transition-colors">
                ...
                <td className="px-5 py-3.5">
                  <span className={`text-[12px] px-2.5 py-1 rounded-lg font-medium ${report.type === "Automated" ? "bg-primary/10 text-primary" : "bg-muted text-muted-foreground"}`}>
                    {report.type}
                  </span>
                </td>
      ...
      <div className="apple-card p-6">
        <h2 className="text-[15px] font-semibold text-foreground mb-4">Scheduled Reports</h2>
        <div className="space-y-3">
          {scheduled.map((s) => (
            <div key={s.name} className="flex items-center justify-between p-4 rounded-2xl bg-muted/30">
              <div>
