      ...
      <div className="asana-card p-6 border-l-4 border-l-destructive">
        <div className="flex items-start gap-4">
          ...
          <div>
            ...
            <h1 className="text-[22px] font-bold text-foreground mt-1 leading-snug">
      ...
      <div className="asana-card p-6">
        ...
        <div className="flex items-center gap-5 mt-3">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[12px] text-muted-foreground">Last week (avg 52%)</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="h-3 w-3 rounded-sm bg-destructive opacity-85" />
            <span className="text-[12px] text-muted-foreground">This week (avg 60%)</span>
          </div>
        </div>
      </div>
          ...
          {contributors.map((item) => (
            <div key={item.label} className="flex items-start gap-4 p-4 rounded-xl bg-muted/30 border border-border/50">
              <div className={`h-10 w-10 rounded-xl flex items-center justify-center shrink-0 ${
                item.severity === "high" ? "bg-destructive/10 text-destructive" : "bg-chart-success/10 text-chart-success"
              }`}>
              ...
              {item.severity === "high" ? (
                <ArrowDownRight className="h-4 w-4 text-destructive shrink-0 mt-1" />
              ) : (
                <CheckCircle2 className="h-4 w-4 text-chart-success shrink-0 mt-1" />
              )}
            </div>
          ))}
        </div>
          ...
          {recommendations.map((rec, i) => (
            ...
            >
              ...
              <Button
                ...
                className="rounded-lg text-[13px] h-8 shrink-0 gap-1.5"
                onClick={() => navigate("/insight/performance-drop/action")}
              >
                {rec.primary ? "Fix Now" : "View"}
